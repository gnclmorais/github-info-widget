...
(function($) {
    $.fn.githubInfo = function (options) {
        ...
        function dateDiff(a, b) {
            ...
            return '<span class="time-ago">(' + diff.value + ' ' + diff.unit + (diff.value > 1 ? 's' : '') +' ago)</span>';
        }
        ...
        return this.each(function() {
            ...
            $.ajax({
                type: 'GET',
                url: 'https://api.github.com/repos/' + options.username + '/' + options.repository,
                dataType: 'json',
                success: function(data) {
                    ...
                    wrapper.append('<div class="stats">' +
                                   '<div class="ownership"><img class="gravatar" src="' + data.owner.avatar_url + '" />' +
                    ...
                    var todayDate  = new Date(),
                        ...
                        createdAgo = dateDiff(createDate, todayDate),
                        updatedAgo = dateDiff(updateDate, todayDate),
                        layout     = options.horizontal ? " horizontal" : "";

                    wrapper.append('<div class="activity' + layout + '">' +
                                   '<p class="history"><span class="emphasis">Commit history</span><span class="sparkline"></span></p>' +
                                   '<p class="updated"><span class="emphasis">Updated</span>' + dateFormatter(updateDate) + ' ' + updatedAgo + '</p>' +
                                   '<p class="created"><span class="emphasis">Created</span>' + dateFormatter(createDate) + ' ' + createdAgo + '</p>' +
                                   '</div>');

                    /** Request repo's commits */
                    $.ajax({
                        type: 'GET',
                        url: 'https://api.github.com/repos/' + options.username + '/' + options.repository + '/commits',
                        dataType: 'json',
                        success: function(data) {
                            var i = options.timespan,
                                commits = [],
                                today = new XDate(),
                                day,
                                diff = 0;
                            ...
                            /** Check dates */
                            i = 0;
                            while (diff < options.timespan && data[i]) {
                                day = new XDate(data[i++].commit.committer.date);

                                diff = Math.floor(day.diffDays(today));

                                if (diff < options.timespan) commits[diff] += 1;
                            }

                            /** Reverse commits order (older to newest */
                            ...
                            /** Latest commits */
                            var divLatestCommits = '<div class="lastest-commits">' +
                            ...
                            for (i = 0; i < length; i++) {
                                divLatestCommits += '<div class="commit ' + (i % 2 ? 'even' : 'odd') + '">' +
                                    ...
                                    '<img class="gravatar" src="' + data[i].committer.avatar_url + '" />' +
                                    '<a class="link-user" href="https://github.com/' + data[i].committer.login + '">' + data[i].committer.login + '</a> ' +
                                    dateDiff(new Date(data[i].commit.author.date), today) +
                                    '</div>' +
